...
/*
    "transfer delta correctly" in {
      ...
      runOn(first) {
        mediator ! headers.Status(versions = Map.empty, isReplyToStatus = false)
      ...
      runOn(first) {
        ...
        mediator ! headers.Status(versions = deltaBuckets1.map(b => b.owner -> b.version).toMap, isReplyToStatus = false)
